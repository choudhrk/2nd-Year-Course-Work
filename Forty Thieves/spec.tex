...
$S$: \texttt{Sequence of type T} \wss{What is the type of the state variable?}
...
\noindent pop():
\begin{itemize}
\item $\mathit{out}: Stack(S[0],...,S[|S|-2])$ \wss{What should go here?}

\item exception: $(Stack = \epsilon) \implies out\_of\_range$ \wss{What should go here?}
...
\item exception: $(S = \epsilon) \implies out\_of\_range$  \wss{What should go here?}
...
\begin{itemize}
\item output: $(0 \le |Deck| \le 64)$ \wss{What goes here?  The deck moves involves moving a card from
...
\begin{tabular}{|p{3cm}|l|}
\hhline{|-|-|}
$c = \mbox{Tableau}$ & T[$n_0$], T[$n_1$] := T[$n_1$].push(T[$n_0$].top()),\wss{What goes here?}\\
\hhline{|-|-|}
$c = \mbox{Foundation}$ & T[$n_0$], F[$n_1$] := F[$n_1$].push(T[$n_0$].top())\wss{What goes here?}\\
\hhline{|-|-|}
\end{tabular}
...
\begin{tabular}{|p{3cm}|l|}
\hhline{|-|-|}
$c = \mbox{Tableau}$ & W, T[$n$] := T[$n$].push(W.top())\wss{What goes here?}\\
\hhline{|-|-|}
$c = \mbox{Foundation}$ & W, F[$n$] := F[$n$].push(W.top())\wss{What goes here?}\\
\hhline{|-|-|}
\end{tabular}
...
\noindent deck\_mv():
\begin{itemize}
\item transition: D, W := W.push(D.top())\wss{What goes here?}
\item exception:
...
\item output: $out := \text{valid\_tab\_mv} \vee \text{valid\_waste\_mv} \vee
  ...
  n_1: \mathbb{N} | (c : T \land (n_0 \in [0..9]) \land (n_1 \in [0..9])) \lor (c : F \land  (n_0 \in [0..9]) \land  (n_1 \in [0..7])) \wss{\text{What goes here?}}: \text{is\_valid\_tab\_mv}(c, n_0, n_1))$\\

  $\text{valid\_waste\_mv} \equiv (\exists c: \text{CategoryT}, n: \mathbb{N} | (c : T \land (n \in [0..63])) \lor (c : F \land (n \in [0..63]))
...
\begin{itemize}
\item output: $(\forall f \in F: |f| = 13)$ \wss{What goes here?}
\item exception: None
\end{itemize}
...
\noindent
$\text{two\_decks}(T, F, D, W) \equiv$ \wss{This function returns True if there is two of each card in the game}
\newline $(\forall \mathit{st}: \text{SuitT}, \mathit{rk}:
\text{RankT} | \mathit{st} \in \text{SuitT} \wedge \mathit{rk} \in \text{RankT}:$
\newline $(\exists (a, b : CardT) : (a.st = b.st)\land (a.rk = b.rk))\wss{\text{What goes here?}})$
...
\rightarrow \mathbb{B}) \rightarrow \mathbb{N}$\\
\noindent $\text{cnt\_cards\_stack}(S, f) \equiv (+ s: \text{CardT} | s \in S: 1)$\\
...
\begin{tabular}{|p{4cm}|p{3.5cm}|l|}
\hhline{|-|-|-|}
$T[n_0].\text{size}() > 0$ & F[$n_1$].size() $>$ 0 & True \wss{What goes here?}\\
\hhline{|~|-|-|}
 & F[$n_1$].size() = 0 & True \wss{What goes here?}\\
\hhline{|-|-|-|}
...
\noindent \wss{What goes here?  You
  may need a table?}\\
...
\noindent tab\_placeable: $\forall n_0, n_1 : CardT \cdot (((n_0).s = (n_1).s) \land (n_0.r + 1 = n_1.r))$ \wss{Complete this specification}\\

\noindent foundation\_placeable: $\forall n_0, n_1 : CardT \cdot (((n_0).s = (n_1).s) \land (n_0.r = n_1.r +1))$ \wss{Complete this specification}

\newpage

\section*{Critique of Design}

Here is my list of critiques for the given specification:

\begin{enumerate}
\item The purpose of some of the local functions were not automatically clear upon reading them. For example the tab\_decks(deck) function. I would have never imagined it to be the  function that sorts the tableau stacks from the complete deck of 104 cards as every other reference of deck means the game's stock pile.

\item I would consider using abstract objects for 
\begin{itemize}
\item Stacks that are each type of card suit. This ensures we'd always have the right amount and type of cards.
\item Each type of tableau as they differ in terms of the stack functions that can be performed upon them (E.g pop cannot be done on foundation stacks). 
\end{itemize}

\item I would also consider adding a undo function to the game board module to make it more complete.
